...
{
    $stmt = db_library()->prepare(
        'SELECT * FROM collections WHERE subject_id = :id'
    );
...
function add_collection(int $subjectId): void
{
    $sql = 'INSERT INTO collections (
            ...
            ON DUPLICATE KEY UPDATE
                collected = TRUE,
                collection_date = COALESCE(collection_date, CURRENT_DATE),
...
function list_collections(int $limit = 50, int $offset = 0): array
{
    ...
    $sql = 'SELECT
            ...
            JOIN ' . $publicDb . '.subjects s
                ON s.id = c.subject_id
            LEFT JOIN cover_cache cc
                ON cc.subject_id = c.subject_id
               AND cc.status = \'cached\'
            WHERE c.collected = TRUE
              AND s.type = 2
