...
{
    return (int) db_public()
        ->query('SELECT COUNT(*) FROM subjects WHERE type = 2')
        ->fetchColumn();
}
...
function get_subject(int $id): ?array
{
    ...
    $sql = 'SELECT s.*, cc.local_path AS cover_local_path
            FROM subjects s
            LEFT JOIN ' . $libraryDb . '.cover_cache cc
                ON cc.subject_id = s.id AND cc.status = \'cached\'
...
{
    $stmt = db_public()->prepare(
        'SELECT COUNT(*) FROM episodes WHERE subject_id = :id AND type = 0'
    );
...
function get_subject_characters(int $subjectId): array
{
    $sql = 'SELECT c.id, c.name, sc.type
            ...
            ORDER BY sc.type, sc.`order`, c.name
            LIMIT 80';
...
{
    return $date && preg_match('/^(\d{4})/', $date, $matches)
        ? $matches[1]
        : '';
}
