...
?>
<section class="detail"><img class="poster" src="<?= cover_url($id, $subject['cover_local_path'] ?? null) ?>" alt=""><div><h1><?= h($subject['name_cn']) ?></h1><h2><?= h($subject['name']) ?></h2><p><a href="collection_edit.php?id=<?= $id ?>">编辑收藏</a></p><?php if ($collection && $collection['collected']): ?><span class="badge">已收藏</span><?php else: ?><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="collect"><button>加入收藏</button></form><?php endif; ?></div></section>
