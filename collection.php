...
?>
<h1>我的收藏</h1>
<div class="grid"><?php foreach ($items as $item): ?><article class="card"><a href="subject.php?id=<?= (int) $item['subject_id'] ?>"><img src="<?= cover_url((int) $item['subject_id'], $item['cover_local_path'] ?? null) ?>" alt=""<?= cover_onerror_attr() ?>></a><h2><?= h($item['name_cn'] ?: $item['name']) ?></h2><p><?= h(subject_year($item['date'])) ?> · 我的评分 <?= h($item['my_rating'] ?? '未评分') ?></p><p><?= h($item['watch_progress'] ?? '') ?></p><a href="collection_edit.php?id=<?= (int) $item['subject_id'] ?>">编辑</a></article><?php endforeach; ?></div>
